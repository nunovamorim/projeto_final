#!/usr/bin/env python3
"""
Simple Ground Station Client for Satellite Simulation
"""
...
class SatelliteData:
    ...
    def update(self, data):
        ...
        # Extract fields from binary data using little-endian format
        try:
            ...
            self.time_history.append(self.timestamp / 1000.0)  # Convert to seconds
...
class GroundStationGUI:
    ...
    def send_command(self):
        ...
        try:
            ...
            # Send command
            self.socket.sendall(data + bytes([checksum]))
    ...
    def update_telemetry_display(self):
        ...
        if len(self.satellite_data.time_history) > 0:
            ...
            self.ax2.plot(self.satellite_data.time_history, self.satellite_data.power_history, 'g-', label='Power (%)')
            
            # Create a second y-axis for temperature
            ax2_temp = self.ax2.twinx()
            ax2_temp.plot(self.satellite_data.time_history, self.satellite_data.temp_history, 'r-', label='Temp (°C)')
    ...
    def log_message(self, message):
        ...
        self.msg_text.insert(tk.END, time.strftime("%H:%M:%S") + " - " + message + "\n")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up
        if app.connected and app.socket:
            app.socket.close()
